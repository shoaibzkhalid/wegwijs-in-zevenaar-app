      ...
      <nb-card class="news-card">
        <nb-card-item>
          <nb-body>
            ...
            <nb-grid v-if="organization.attributes.street" style="margin-bottom: 10">
            ...
            <nb-grid v-if="organization.attributes.phone_number" style="margin-bottom: 10">
              <nb-col style="width: 30"
            ...
            <nb-grid v-if="organization.attributes.email" style="margin-bottom: 10">
              <nb-col style="width: 30"
              ...
              <nb-col
                ><TouchableOpacity :onPress="() => openEmail()" :activeOpacity="1">
                  <nb-text>{{ organization.attributes.email }}</nb-text></TouchableOpacity
                ></nb-col
              >
            </nb-grid>

            <nb-grid v-if="organization.attributes.website" style="margin-bottom: 10">
              <nb-col style="width: 30"
              ...
              <nb-col
                ><TouchableOpacity :onPress="() => openWebsite()" :activeOpacity="1">
                  <nb-text>{{ organization.attributes.website }}</nb-text></TouchableOpacity
                ></nb-col
              >
            </nb-grid>
          </nb-body>
  ...
  methods: {
    dialCall() {
      ...
      if (Platform.OS === 'android') {
        phoneNumber = 'tel:${' + this.organization.attributes.phone_number + '}'
      } else {
        phoneNumber = 'telprompt:${' + this.organization.attributes.phone_number + '}'
      }
    ...
    openMaps() {
      const address =
        ...
        (this.organization.attributes.postal_code
          ? this.organization.attributes.postal_code + ' '
          : '') +
        (this.organization.attributes.city ? this.organization.attributes.city : '')
